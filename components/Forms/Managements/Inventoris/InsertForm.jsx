...
import { insertInventoris } from '@/services/management/inventorisService';
...
const InsertForm = ({ token }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await insertInventoris(token, inventoris);
      setIsLoading(false);
      setSuccess();
    } catch (error) {
      console.error(error.message);
        ...
        <div className='flex flex-col gap-y-3'>
          <div className='flex flex-row gap-x-3'>
            ...
            <Select
              ...
              {anakAsuhanList.map((item) => {
                return (
                  <option value={item.id} selected={item.id === anakAsuhan}>
                    {item.nama}
                  </option>
                );
              })}
          ...
          <div className='flex flex-row gap-x-3'>
            <Select
              ...
              {barangList.map((item) => {
                return (
                  <option value={item.id} selected={item.id === barang}>
                    {item.nama}
                  </option>
                );
              })}
