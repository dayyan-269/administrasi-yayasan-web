...
import { insertJenisPembayaran } from '@/services/master/jenisPembayaranService';
...
const InsertForm = ({ token }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await insertJenisPembayaran(token, data);
    ...
    <>
      ...
      {infoMessage ? (
        <PrimaryAlert className='alert-success'>
          Tambah data jenis pembayaran berhasil.
        </PrimaryAlert>
      ) : null}

      {errorMessage ? (
        <PrimaryAlert type='error' className='alert-error'>
          Tambah data jenis pembayaran gagal.
        </PrimaryAlert>
      ) : null}
