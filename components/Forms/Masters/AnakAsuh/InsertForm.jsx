...
import { insertAnakAsuhan } from '@/services/master/anakAsuhanService';
...
const InsertForm = ({ token }) => {
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      console.log(anakAsuhan);
      await insertAnakAsuhan(token, anakAsuhan);
      setIsLoading(false);
      setSuccess();
    } catch (error) {
      console.error(error.message);
    ...
    <>
      ...
      {infoMessage ? (
        <PrimaryAlert className='alert-success'>
          Tambah data anak asuhan berhasil.
        </PrimaryAlert>
      ) : null}

      {errorMessage ? (
        <PrimaryAlert type='error' className='alert-error'>
          Tambah data anak asuhan gagal.
        </PrimaryAlert>
      ) : null}
